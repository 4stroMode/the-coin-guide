...
class $modify(CoinGuideLevelSearchLayer, LevelSearchLayer) {
    ...
    void onFreeCoins(CCObject*) {
        auto loading = FLAlertLayer::create("Loading", "Fetching free coin guides...", "OK");
        loading->show();

        Database::fetchFreeCoinLevels([loading](std::optional<std::vector<int>> result) {
            geode::queueInMainThread([loading, result]() {
                loading->keyBackClicked();
                if (result.has_value() && !result.value().empty()) {
                    std::string idStr = "";
                    for (size_t i = 0; i < result.value().size(); ++i) {
                        idStr += std::to_string(result.value()[i]);
                        if (i < result.value().size() - 1) idStr += ",";
                    }
                    auto searchObj = GJSearchObject::create(SearchType::Type19, idStr);
                    auto browserScene = LevelBrowserLayer::scene(searchObj);
                    
                    if (browserScene->getChildrenCount() > 0) {
                        if (auto browserLayer = typeinfo_cast<CCLayer*>(browserScene->getChildren()->objectAtIndex(0))) {
                            if (auto titleNode = browserLayer->getChildByID("title-label")) {
                                if (auto titleLabel = typeinfo_cast<CCLabelBMFont*>(titleNode)) {
                                    titleLabel->setString("Coin Guide: Free Coins");
